...
| Broken authentication/session theft | Root APIs require the `casaos` access-token issuer, so a same-key `refresh` token is rejected; management and v3 file credentials are accepted only from one `Authorization: Bearer` header, and credential-shaped query parameters are rejected before authentication. External services still own issuance, enrollment, revocation, and password policy. Password guessing, initial setup left open, and stolen JWTs remain stack-level risks. | Create admin before exposure; rate-limit all credential/token routes; prefer edge MFA; use TLS only; short sessions/revocation where supported; never log query strings; migrate clients from `?token=` to the Authorization header; test issuer confusion, logout, and revocation across every component. |
...
| SMB parser, credential, or downgrade attack | SMB discovery runs only in a same-binary child which, when the service starts as root, drops to `nobody`, removes supplementary groups, applies process/resource limits, proves that sandbox with a fixed `READY` handshake, and only then receives bounded credentials over stdin. Credentials are absent from argv/environment; stdout/stderr, time, concurrency, share count, and names are bounded. Discovery and server configuration require SMB signing, and kernel CIFS mounts use `sign` with port 445 only. Guest/anonymous shares and privileged `force user` mappings are disabled. The root build pins the reviewed ReCasaOS SMB2 fork tag and CI rejects the vulnerable hiro path, CloudSoda paths, SDDL, replacements, and version drift. | Run the service only under the reviewed unit and keep Samba patched. The helper must run as UID/GID 65534 with no supplementary groups; every other non-root identity fails closed. Root/the service UID and the kernel remain trusted; the handshake narrows but cannot eliminate every pre-exec host-local inspection race. Verify the fork release provenance and tag CI, signed discovery, signed mount compatibility, child limit tests, and negative malformed-server cases on each supported Linux release. |
...
| Upload/download denial of service | Root multipart handlers cap chunks at 256 MiB, files at 64 GiB/256 chunks, and active v1/v2 sessions at 16 per registry. Completed replay tombstones are bounded at 128, expiry cleanup is retryable, indexes are checked before allocation, and final publication binds the assembled file's full stat identity plus SHA-256. Assembly streams and upload paths resolve through pinned roots. Each v1/v2 session also pins its staging directory and parent once and resolves chunk publication, chunk-set probes, assembly reads and writes, and the final target commit through those held descriptors; cleanup proves the name still identifies the pinned directory inode, quarantines it, and never recursively deletes a name that no longer matches. The candidate also checks `f_bavail` through a descriptor for the actual staging write parent beneath the pinned roots, reserves each admitted chunk/final assembly under a process-wide lock, and preserves 512 MiB of unprivileged free headroom. The eventual target can be on a different nested mount and is not the capacity source for staging writes. After v2 assembly admission fails, chunk probes request a retry and a verified duplicate chunk retries publication without rewriting the chunk; success requires a published target. Durable per-principal quotas and restart persistence remain concerns; a hostile local-parent rename is bounded to a fail-closed conflict while the session holds its descriptors, but it can still strand a staging tree for operator review. | Keep privileged upload APIs on VPN; add durable per-user/disk quotas and crash-safe reservations; keep upload parents unavailable to untrusted local writers; monitor private temporary uploads; test 413/429/507, interruption, concurrency, replay, archive budgets, and disk exhaustion. |
...
| Cloud OAuth code injection or token theft | Cloud recovery remains disabled and providers stay unadvertised from the root service. The staged `pkg/oauthsecurity` package (deliberately without a production call site, enforced by test) implements single-use, expiring state bound to the authenticated principal and the exact allowlisted redirect URI, retained only as SHA-256 digests; mandatory S256 PKCE where a provider cannot use PKCE stays unsupported; strict runtime provider configuration whose client secrets come only from the environment or a root-owned 0600/0400 credential file, never from source; canonical exact-HTTPS endpoint and redirect validation; and a bounded exchange client that posts only to the configured token URL, never follows redirects, disables ambient proxies and compression, requires TLS 1.2, and redacts every failure. | Keep Issue #11 open. Wire any provider only after the complete flow is reviewed: sealed refresh-token storage with rotation and revocation, rclone token delivery without plaintext persistence, provider-specific error redaction, and replay/concurrency tests against a real provider. Never restore the legacy unauthenticated callback that accepted a bare authorization code. |
...
- Issue #25's bounded worker protocol is implemented as a release candidate,
  but the issue remains open until exact-head Linux runs both the real
  loopback-TCP NBD/D-state case and the recoverable same-host FUSE-backed
  NBD/D-state case to prove timeout, quarantine, cgroup cleanup, restart,
  cancellation, and resource-headroom behavior. Cross-host remote-block,
  permanent-failure, and exact-target cases remain separate gates. A
  dedicated CI job boots the clean exact commit under PID 1 in a
  checksum-pinned, pristine Debian 11/systemd 247 unified-cgroup-v2 QEMU TCG VM
  and runs the live isolation suite; a skipped or failed job is not
  ...
  The candidate also places the NBD backing image behind an unprivileged
  `bindfs` FUSE daemon, verifies its exact PID/start time/executable, all four
  `/proc` `Uid:` fields equal to the unprivileged runner UID, empty effective
  capabilities, restricted mount options, and unique `fusectl` connection,
  then stops only that daemon through its
  recorded pidfd. The NBD daemon and loopback listener must remain live while
  `fusectl` reports at least one waiting kernel request and four workers enter
  real D-state. The same bounded timeouts, four-worker quarantine, fifth-request
  fast rejection, static-page availability, management sentinel, pending
  restart, PID 1 reparenting, explicit resume, request drain, and byte-correct
  recovery assertions remain mandatory. Cleanup resumes FUSE before NBD,
  stops the units, unmounts ext4, removes device-mapper, disconnects NBD,
  terminates the exact NBD process, unmounts FUSE, terminates the exact FUSE
  process, and finally removes only a test-owned empty `fusectl` mount.
  This closes the earlier parser-only evidence gap only for exact heads on
  which the job succeeds. It does not close Issue #25 or prove a cross-host
  network partition, a permanent remote-block failure, or exact target-host
  recovery. Ubuntu 24.04/systemd 255 evidence alone does not satisfy the
  compatibility-floor gate.
...
- management and v3 file query-string token flows are rejected; any new credential transport must remain header-only or receive an explicit security review;
