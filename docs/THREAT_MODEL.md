...
| Public bearer or verifier exposure | The supported candidate never provisions or persists the raw bearer as a server credential; authorized requests still deliver it transiently in the `Authorization` header. An administrator generates an `rc1_` plus base64url-no-pad encoding of 32 random bytes on an independent workstation, keeps its durable copy only in a password manager, and installs only `recasaos-public-verifier-v1:sha256:<64 lowercase hex>` followed by one LF through the dedicated verifier setting. When the portal is enabled, an empty legacy `RECASAOS_PUBLIC_FILE_TOKEN_FILE` value is treated as unset and any non-empty value fails closed, including simultaneous verifier configuration. A disabled portal ignores the legacy setting so an old template cannot terminate the management daemon. A disclosed verifier does not satisfy bearer authentication, but it remains protected configuration and can expose rotation state. | Load the strict verifier through a systemd credential, set `LimitCORE=0`, reject malformed, linked, ambiguously named, or broadly accessible verifier files, and never log bearer or verifier material. Keep Issue #26 open until bidirectional bind-alias, legacy-setting rejection, atomic verifier publication, restart, old-bearer rejection, new-bearer acceptance, and rollback tests pass. The raw bearer must never enter host configuration, shell history, durable storage, or logs; treat edge and server request memory as part of the live credential boundary. |
...
- Issue #26 must prove that verifier-only provisioning rejects every legacy
  raw-token configuration and completes bind-alias-safe rotation and rollback;
