...
| Browser download credential or memory leak | The public portal bearer now lives only in current-page memory. A scoped Worker consumes a short-lived nonce bound to the exact top-level portal client/path/URL, requests the bearer once over a dedicated message port, strips the non-secret fragment before one same-origin header-authenticated fetch, rejects redirects, and passes the response stream through untouched. Unsupported clients use a one-at-a-time, 32 MiB payload-bounded reader. Worker state, timeout, client, replay, or message failures return an empty navigation response rather than falling through to an unauthenticated file fetch. Transparent retry/resume is intentionally not claimed because the nonce is one-shot. | Keep Issue #20 open until real HTTPS Chromium/Firefox/WebKit tests verify no bearer in URL/referrer/history/storage/logs, byte/filename round trips, bounded memory, two-tab isolation, restart/replay/logout/rotation failures, redirect denial, Range, retry/resume, and cancellation. Rotation cannot revoke an already handed response. Do not call a target host public-ready from static or unit tests. |
| Raw public bearer exposed through a bind alias | The current token file is no-follow, single-link, service-owned, and mode-restricted, but its separation from the public root is checked lexically. Bind mounts can expose one underlying tree under unrelated pathnames, allowing a configuration to pass that check while the raw bearer remains reachable from the share. | Treat bind-mounted public roots or token hierarchies as unsupported. Keep Issue #26 open until supported provisioning stores only a precomputed verifier on the server, rejects legacy raw-token ambiguity, and passes bidirectional bind-alias plus rotation tests. Never use a path-string comparison as public-readiness evidence. |
| Blocking public root filesystem | Before returning a usable portal, the public share records `statx` mount ID and `fstatfs` type from its already-pinned descriptor. Only ext2/3/4, XFS, Btrfs, tmpfs, and F2FS are eligible; FUSE, network, overlay, ZFS, and unknown types fail closed. Every later descriptor-relative open must report the captured mount ID, in addition to `RESOLVE_NO_XDEV`. Unsupported roots therefore never enter the in-process download-slot pool. An allowlisted filesystem can still be backed by a remote or failing block device, and the listener remains in the privileged daemon. | Keep Issues #22 and #25 open. Keep the mount namespace and storage stack operator-controlled. The allowlist is an eligibility policy, not proof that the kernel, disk, or underlying block device cannot hang; initial path lookup and even filesystem classification can block before startup completes. ZFS and other excluded local filesystems require dedicated compatibility evidence and a reviewed policy change, never an unrestricted fallback. Mount tests require an explicit opt-in and root, prove they are outside PID 1's mount namespace, reject remaining shared propagation, and run only in the dedicated ephemeral CI job under `unshare --mount --propagation private`. Separate the public handler and potentially blocking work from the privileged daemon before claiming production public readiness. |
...
- Issue #22 remains open until the staged filesystem gate has exact-HEAD Linux
  ...
  disposition;
- Issue #25 must separate the Internet-facing listener and potentially
  blocking filesystem work from the privileged management daemon;
- Issue #26 must replace raw bearer files with verifier-only provisioning and
  prove bind-alias-safe rotation;
