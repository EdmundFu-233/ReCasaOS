...
| Browser download credential or memory leak | The supplied client does not intentionally write the bearer to persistent browser storage or a URL. A scoped Worker records a short-lived nonce and a separate in-memory frame proof bound to the exact top-level portal client/path/URL. A minimal same-origin transport document relays an origin-checked message with a transferred one-use port so the Worker can bind the reservation to the exact nested-frame client before navigation; the proof never enters the URL and is erased after binding. The page then gives that document an origin-checked command and the proven frame initiates its own navigation, allowing the Worker to require the widely implemented `FetchEvent.clientId` to equal the bound frame. A non-empty `replacesClientId` is only an additional consistency check because current engines do not implement it. The Worker requests the bearer once from the original page over a dedicated message port, strips the non-secret nonce fragment before one same-origin header-authenticated fetch, rejects redirects, and passes the response stream through untouched. The bearer necessarily exists transiently in page, Worker, request-header, edge, and server memory. Unsupported or rejected navigation clients fail inside the disposable frame and use a one-at-a-time, 32 MiB payload-bounded reader while the main page remains intact. Worker state, timeout, client, replay, or message failures return an empty navigation response rather than falling through to an unauthenticated file fetch. The portal withholds `Last-Modified` and strong `ETag` values; a weak or unknown `If-Range` therefore falls back to a complete `200`, while an ordinary single initial `Range` remains `206`. Transparent retry/resume is intentionally not claimed because the nonce is one-shot. | Keep Issue #20 open until real HTTPS Chromium/Firefox/WebKit tests verify no bearer retention in URL/referrer/history/storage/DevTools/proxy/application logs or crash artifacts, byte/filename round trips, bounded memory, two-tab isolation, restart/replay/logout/rotation failures, redirect denial, initial Range, retry/resume, and cancellation. The isolated Playwright smoke records one authenticated upstream initial-Range request and one `206` per bundled engine, but does not prove stable retail browsers, transparent retry/resume, or a target host. Rotation cannot revoke an already handed response. Do not call a target host public-ready from static or unit tests. |
