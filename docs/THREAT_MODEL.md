...
| File path or authorization flaw | Direct privileged file reads, writes, uploads, downloads, archives, rename, delete, thumbnail, listing, and size/count operations are confined to startup-pinned roots with descriptor-relative Linux `openat2`; links and special files fail closed, and destructive recursion has mount/depth/entry limits. The legacy asynchronous copy/move worker still uses pathname helpers and remains tracked by issue #7. The public portal uses a separate read-only pinned share with stricter hidden-name, hardlink, and mount-crossing rules. | Public edge exposes only `/public-files`; keep every privileged API on VPN; configure only operator-owned roots; trust and monitor the mount namespace; do not grant untrusted principals rename/write access to management parents; test traversal, symlinks, hardlinks, mount boundaries, races, and cross-user access. Independent deployment review remains required. |
...
| Upload/download denial of service | Root multipart handlers cap chunks at 256 MiB, files at 64 GiB/256 chunks, and active v2 sessions at 16 with expiry cleanup; indexes are checked before allocation, assembly streams, and upload paths resolve through pinned roots. Aggregate disk quota and hostile local-parent rename races remain concerns. | Keep privileged upload APIs on VPN; add per-user/disk quotas and reserved system space; keep upload parents unavailable to untrusted local writers; monitor private temporary uploads; test 413/429, interruption, concurrency, archive budgets, and disk exhaustion. |
...
pinned descriptors. Issue #7 remains open for the asynchronous copy/move worker,
mount-aware policy refinement, and hostile local-parent rename semantics; those
limitations reinforce the private/VPN-only management boundary.
