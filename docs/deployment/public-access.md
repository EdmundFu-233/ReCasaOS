...
production public-readiness blocker until the exact-head qualification job
passes both recoverable NBD and same-host FUSE-backed NBD phases, and separate
cross-host remote-block, permanent-failure, and exact target-host evidence
proves the complete deployment boundary.
...
job is configured to create a dedicated 256 MiB raw image beneath an
unprivileged `bindfs` FUSE source, consume only its restricted FUSE-mounted
alias from an unprivileged `qemu-nbd` process bound to guest `127.0.0.1`, attach
the export to the kernel's exact `/dev/nbd0`, and place a device-mapper linear
target and ext4 filesystem above it. The portal therefore still pins an
allowlisted ext4 root; the FUSE fault is deliberately beneath its block path,
not a policy bypass that mounts FUSE as the public root. `bindfs` runs in the
foreground as the non-root test identity with `--no-allow-other`, zero entry and
attribute cache timeouts, `nodev,nosuid,noexec`, no capabilities, and a single
recorded `fusectl` connection.

After the production portal has pinned that root, the job runs two recoverable
same-guest fault phases. It first drops the guest's clean caches and stops only
the recorded NBD server through a pidfd. It then fully recovers and repeats the
test while leaving the exact NBD process, loopback listener, device-mapper, and
ext4 mount active and stopping only the recorded FUSE daemon through its pidfd.
The FUSE phase must observe that exact daemon in `T`, the NBD process still
runnable, and a numeric `fusectl` `waiting` value of at least one before it may
accept the storage stall as real kernel evidence.

In each phase four concurrent authenticated directory operations must enter
...
four-worker quarantine threshold without creating a fifth child. The fifth
request must fail quickly from admission quarantine, while the static portal
page remains available. The job records live unit task and memory headroom,
proves that the independent management sentinel is unchanged, and requests a
non-blocking portal restart. systemd must remain in deactivation behind the
four reparented D-state children until an explicit exact-process pidfd
`SIGCONT`; after that operator action the old workers must disappear, the FUSE
kernel waiting count must drain to zero in the FUSE phase, a new portal
invocation must pass the isolation checks, and the original file bytes must be
readable.

Cleanup fails closed unless it can first resume FUSE and then NBD by their
recorded PID/start-time identities. It then stops the units, unmounts ext4,
removes only the recorded mapper, disconnects `/dev/nbd0`, terminates the exact
NBD process, unmounts the exact FUSE mount, terminates the exact FUSE process,
proves that no FUSE connection remains, and removes `fusectl` only when the test
itself mounted that empty control filesystem.

These are recoverable same-host loopback-NBD and FUSE-backed NBD cases with real
kernel D-state. They do not prove iSCSI, a cross-host network partition, a
permanently failed remote-block device, every kernel fault, or the exact target
...
> worker capacity, resource headroom, and both isolated recoverable
> loopback-NBD and same-host FUSE-backed NBD D-state scenarios described above.
> Its eight-worker aggregate
...
> job passes: it is only the recoverable same-host qualification. Cross-host
> remote-block behavior, permanent-failure handling, and the exact target-host
> recovery matrix remain separate release gates. Until the candidate runs green
> on its exact head, neither recoverable fault phase is evidence either.
...
6. Keep the no-query access-log format and the supplied runtime-error control:
   ...
   query-free.
7. Keep the final 404 deny and the enforced 1 MB request-body ceiling. Caddy
   rejects a POST/PUT/PATCH without a declared Content-Length with 411 and
   any declared Content-Length at or above 1000000 bytes with 413, both
   before proxying; Nginx rejects bodies above `client_max_body_size` with
   413. A body without a declared length on GET/HEAD still reaches the
   portal, which answers without reading it. Portal downloads
...
| Resource bounds | Eight concurrent storage workers succeed while a ninth request returns 503 with `Retry-After`; `TasksCurrent` and `MemoryCurrent` remain below the reviewed unit headroom. Record `MemoryPeak` when the manager exposes it; systemd 247 instead requires separately reviewed guest-side peak sampling or equivalent version-appropriate evidence, and the missing property is not itself a pass. Workers expose neither the AF_INET listener nor raw bearer, all non-stdio descriptors are close-on-exec, and normally killable workers are reaped after cancellation and coordinator SIGKILL. Injected pidfd-signaling failure must close admission and retain capacity until reap. Both isolated recoverable loopback-TCP NBD and same-host FUSE-backed NBD D-state phases must prove the four-worker quarantine threshold, `TasksMax=256`, `MemoryMax=512M`, `MemorySwapMax=0`, static and management health, pending restart behavior, and explicit-resume recovery. The FUSE phase must additionally prove a live NBD path and a positive kernel `waiting` count during the stall, followed by zero waiting requests and byte-correct reads after recovery. Cross-host remote-block and permanent-failure cases must still reproduce the relevant bounds before public exposure. `LimitNOFILE` and worker RLIMITs are per-process rather than an aggregate cgroup FD cap. Oversized directory and oversized declared-length request-body tests fail with 413 on both edges (length-less Caddy POST/PUT/PATCH fail with 411); Nginx rate/connection limits or the separately reviewed Caddy-fronting limiter are exercised. |
