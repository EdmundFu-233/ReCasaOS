...
// for this expand-only release. The legacy DAO still performs whole-row Save,
// so allowing any sealed or migration-marked row to reach it could erase a
// future envelope. The cutover release must replace this gate atomically with
// its complete legacy/sealed classifier before it writes the first envelope.
func verifyLegacyOnlySMBCredentialState(database *gorm.DB) error {
	var nonLegacyRows int64
	if err := database.Raw(`SELECT count(*) FROM o_connections
		WHERE (credential_id IS NOT NULL AND credential_id <> '')
			OR (credential_format IS NOT NULL AND credential_format <> '')
			OR (password_envelope IS NOT NULL AND (
				typeof(password_envelope) <> 'blob'
				OR length(CAST(password_envelope AS BLOB)) <> 0
			))
			OR row_revision <> 0`).Scan(&nonLegacyRows).Error; err != nil {
